...
describe 'Rake task spree_travel_flight:load' do
  ...
  context 'inserts correctly' do
    ...
    it 'the option values' do
      Rake::Task['spree_travel_flight:load'].invoke
      expect(Spree::OptionValue.where(:name => 'bussiness').count).to be_equal(1)
      expect(Spree::OptionValue.where(:name => 'first').count).to be_equal(1)
      expect(Spree::OptionValue.where(:name => 'economy').count).to be_equal(1)
    end
  end
end